...
#if 0
template <typename T>
void ReadData(const std::string& varName, adios2::IO& io, adios2::Engine& reader, std::vector<T>& res)
{
...
MeowIt(const std::string& varName, adios2::IO& io, adios2::Engine& reader, std::vector<T>& res)
{
  auto var = io.InquireVariable<T>(varName);
  auto shape = var.Shape();
...
//  ReadData(varName, io, reader, data);
  auto dataAH = vtkm::cont::make_ArrayHandle(res, vtkm::CopyFlag::On);
  vtkm::cont::printSummary_ArrayHandle(dataAH, std::cout);
  return vtkm::cont::UnknownArrayHandle(dataAH);
}

template <typename T>
vtkm::cont::UnknownArrayHandle
Bummy(const std::string& varName, adios2::IO& io, adios2::Engine& reader, vtkm::cont::UnknownArrayHandle& arr)
{
  std::vector<T> data;
  arr = MeowIt(varName, io, reader, data);
}

#define TypeTemplateMacro(call) \
  switch(varType[0])            \
  {                             \
    case 'i' :                  \
    {                           \
...
vtkm::cont::UnknownArrayHandle
ReadVar(const std::string& varName, adios2::IO& io, adios2::Engine& reader)
{
  vtkm::cont::UnknownArrayHandle arr;

#if 0
  std::string varType = io.VariableType(varName);
  TypeTemplateMacro(Bummy(varName, io, reader, arr));

  if (varType[0] == 'i')
  {
    TypeTemplateMacro(Bummy<ADIOS_TT>(varName, io, reader));
//    arr = Bummy<int>(varName, io, reader);
    std::vector<int> data;
    arr = MeowIt(varName, io, reader, data);
//    ReadData(varName, io, reader, data);
//    auto dataAH = vtkm::cont::make_ArrayHandle(data, vtkm::CopyFlag::On);
//    vtkm::cont::printSummary_ArrayHandle(dataAH, std::cout);
...
static inline void
TemplateMagic(const std::string& varName, adios2::IO& io, adios2::Engine& engine, vtkm::cont::UnknownArrayHandle& arr)
{
  std::string varType = io.VariableType(varName);
  if (varType == "int")
    Bummy<int>(varName, io, engine, arr);
  else if (varType == "double")
    Bummy<double>(varName, io, engine, arr);
}
